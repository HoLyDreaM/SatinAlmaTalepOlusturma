...
public partial class ParcaliSiparis : System.Web.UI.Page
{
        ...
        if (!string.IsNullOrEmpty(Kontrol))
        {
            ...
            for (int i = 0; i < EvrakNoParcala.Length; i++)
            {
                ...
                if (SiparisDurum == false)
                {
                    ...
                    string PMiktar = Convert.ToString(Convert.ToDecimal(SiparisMiktarBul(Convert.ToInt32(TalepIDParcala[i].ToString()))));
                    txtMiktar[i].Text = PMiktar.ToString().Replace('.', ',');
    ...
    protected void btnParcaliSiparis_Click(object sender, EventArgs e)
    {
        //ihtiyacNo = Session["TalepEvrakNumaralari"].ToString();
        //MalKodu = Session["MalKodlari"].ToString();
        //AciklamaSatirimiz = Session["AciklamaSatirlarimiz"].ToString();
        ...
        for (int i = 0; i < EvrakNoParcala.Length; i++)
        {
            ...
            {
                string MalKodu2 = txtMalKodu[i].Text;
                ...
                decimal Kdv = Convert.ToDecimal(KKdv.ToString());
                ...
                string MasrafMerkezi = MasrafMerkeziBul(EvrakNo, MalKodu2);
                ...
                Stk002Ekle(MalKodu2, TarihRakam, HesapKodu, SiparisNumarasi, Miktar, Fiyat, Tutar, KdvTutari,
                    SEQNO, Saatimiz, PcAdi, EvrakNo, MasrafMerkezi, Aciklama.ToString().Replace("'", "''"),
                    Convert.ToDecimal(DovizKuru), DovizCinsi, Convert.ToDecimal(DovizToplami), Convert.ToInt32(IslemTipi),
                    DepoKod, Convert.ToString(Convert.ToInt32(Kdv)));

                Stk004Guncelle(TarihRakam, Saatimiz, GirenKodu, Miktar, MalKodu2);

                string CariUnvan = drpCariAdi.SelectedItem.Text.ToString().Replace("'", "''");

                string SipNoYeni = SiparisNumarasiBuluyoruz(EvrakNo);
                string CariHesapAdiYeni = CariHesapAdiBul(EvrakNo);

                SiparisKartlariGuncelle(EvrakNo, SiparisNumarasi, Miktar, CariUnvan, Fiyat, SipNoYeni, CariHesapAdiYeni);
            }
        }
        ...
        for (int i = 0; i < EvrakNoParcala.Length; i++)
        {
            ...
            {//kaldıgım yer burası
                ...
                decimal Miktar2 = Convert.ToDecimal(MMdiktar.ToString().Replace(',', '.'));
                SiparisMiktarimiz = SiparisMiktariBul(EvrakNo, MalKodu);
                decimal SpMMiktar = EklenenMiktarimiz(EvrakNo);
                decimal SipMm = SiparisMiktarimiz - SpMMiktar;

                if (Miktar2 == SipMm)
                {
                    ParcaliTalepSiparisDurumGuncelle(7, i);
                }
                else
                {
                    ParcaliTalepSiparisDurumilkGuncelle(i);
                }
            }
        }
    ...
    decimal DovizKuru, string DovizCinsi, decimal DovizTutari, int ParaBirimi, string DepoKodu, string KdvOran)
    {
        ...
        string insertStk = "INSERT INTO STK002(STK002_MalKodu, STK002_IslemTarihi, STK002_GC, STK002_CariHesapKodu, STK002_EvrakSeriNo, STK002_Miktari, STK002_BirimFiyati, " +
                      ...
                      "'" + DovizKuru + "', '" + Aciklama.ToString().Replace("'", "''") + "', N'','" + DepoKodu.ToString() + "', N'', N'', N'', N'', N'', N'', N'', N'', '0.00', '0.00', N'',  " +
    ...
    private void SiparisKartlariGuncelle(string EvrakNo, string SiparisNumarasi, decimal SiparisMiktari, string CariUnvani, 
        decimal BirimFiyati,string SipNoYeni,string CariHesapAdYeni)
    {
        ...
        if (SipNoYeni == "S" || SipNoYeni == "")
        {
            YenSipSorgu = "'" + SiparisNumarasi + "',";
        }
        else
        {
            YenSipSorgu = "'" + SipNoYeni + "/" + SiparisNumarasi + "',";
        }

        if (CariHesapAdYeni == "C" || CariHesapAdYeni == "")
        {
            CarYenSorgu = "'" + CariUnvani + "',";
        }
        else
        {
            CarYenSorgu = "'" + CariHesapAdYeni + "/" + CariUnvani + "', ";
        }

        string Sorgu = "UPDATE Tlp SET  " +
                        "SiparisNo=" + YenSipSorgu + "" +
                        "CariUnvan=" + CarYenSorgu +
    ...
    string AcikSatirimiz, string CariKodu, decimal Tutar, decimal Oran, int ParaBirimi)
    {
        if (DbConnLink.State == ConnectionState.Closed)
            DbConnLink.Open();
        ...
        string insertCar005 = "INSERT INTO CAR005(CAR005_Secenek, CAR005_FaturaTarihi, CAR005_FaturaNo, CAR005_BA, CAR005_CariIslemTipi, CAR005_SatirTipi, CAR005_SatirNo, " +
                      ...
                      "CAR005_EFaturaReferansNo, CAR005_ParaBirimi, CAR005_DovizCinsi, CAR005_DovizKuru, CAR005_DovizTutari, CAR005_TeslimCHKodu) " +
                      "VALUES (6," + Tarih + ",'" + FaturaNo.ToString() + "', N'A', 4,'" + SatirTipi.ToString() + "'," + SatirNo + ", " +
                      "'" + SatirKodu.ToString() + "','" + Filler.ToString() + "','" + AcikSatirimiz.ToString().Replace("'", "''") + "','" + CariKodu.ToString() + "','" + Tutar.ToString().Replace(',', '.') + "', " +
    ...
    private void ParcaliTalepSiparisDurumGuncelle(int Durum,int i)
    {
        if (DbConnUser.State == ConnectionState.Closed)
            DbConnUser.Open();
        ...
        SiparisNums = SiparisNums.PadLeft(6, '0');
        string SiparisNumarasi = SiparisSeriNo + SiparisNums;
        SiparisDurum = chkTalepKapat.Checked;
        int SipDurumu = 0;
        string SipOnayDrm = "";
        string GitDepartman = "";
        GitDepartman = AnaDepartmanKodu();

        if (SiparisDurum == true)
        {
            SipDurumu = 2;
            SipOnayDrm = "7";
        }
        ...
        string GuncellemeSorgusu = "UPDATE Tlp SET " +
            ...
            //"SiparisNo = '" + SiparisNumarasi.ToString() + "', " +
                                     "SipMiktar = SipMiktar+" + Convert.ToDecimal(Miktar.ToString()) + ", " +
                                     "SiparisDurum = " + SipDurumu + ", " +
                                     "ilkIslem = 1 " +
    ...
    private decimal EklenenMiktarimiz(string EvrakNo)
    {
        if (DbConnUser.State == ConnectionState.Closed)
            DbConnUser.Open();

        string sorgu = "SELECT SipMiktar FROM Tlp WHERE EvrakNoTarih+EvrakNo='" + EvrakNo + "'";
    ...
    private void ParcaliTalepSiparisDurumilkGuncelle(int i)
    {
        if (DbConnUser.State == ConnectionState.Closed)
            DbConnUser.Open();
        ...
        else
        {
            SipDurumu = 1;
            SipOnayDrm = "14";
            GitDepartman = Session["DepartmanKodu"].ToString();
        }
    ...
    protected void btnTopluSiparisAc_Click(object sender, EventArgs e)
    {
        for (int i = 0; i < EvrakNoParcala.Length; i++)
        {
            if (string.IsNullOrEmpty(txtFiyat[i].Text))
            ...
            else
            {
                ...
                string Aciklama = txtAciklama[i].Text;
                decimal Kdv = Convert.ToDecimal(txtKdv[i].Text.Replace(',', '.'));
                decimal Miktar = Convert.ToDecimal(txtMiktar[i].Text.Replace(',', '.'));
                decimal Fiyat = Convert.ToDecimal(txtFiyat[i].Text.Replace(',', '.'));
                decimal KdvTutari = (Miktar * Fiyat) * Kdv / 100;
                ...
                DateTime obj = new DateTime();
                obj = Convert.ToDateTime(DateTime.Now.ToString("dd-MM-yyyy"));
                string str;
                Tarih = 0;
                Tarih = obj.ToOADate();
                str = Tarih.ToString().Substring(0, 5);
                ...
                //Stk002Ekle(MalKodu, TarihRakam, HesapKodu, SiparisNumarasi, Miktar, Fiyat, Tutar, KdvTutari, SEQNO, Saatimiz, GirenKodu, EvrakNo, MasrafMerkezi,Aciklama);
            }
        }
    ...
    private void BaglantilariKapat()
    {
        ...
        //DbSiparisDurumu.Dispose();
        DbSiparisDurumu.Close();
    ...
    {
        string CariHesapAdi = txtCariHesapAdi.Text;
        ...
        //sqlCariHesaplar.SelectCommand = null;
        //sqlCariHesaplar.SelectCommand = sorgu;
        //SiparisTablosuEkle();
    }

    private decimal SiparisMiktarBul(int TalepID)
    {
        if (DbConnUser.State == ConnectionState.Closed)
            DbConnUser.Open();

        string sorgu = "SELECT ISNULL(ISNULL(Miktar,0)-ISNULL(SipMiktar,0),0) AS Miktar FROM Tlp " +
                     "WHERE TalepID=" + TalepID + "";
    ...
    private string SiparisNumarasiBuluyoruz(string EvrakNumarasi)
    {
        if (DbConnUser.State == ConnectionState.Closed)
            DbConnUser.Open();

        string sorgu = "SELECT ISNULL(SiparisNo,'S') AS SiparisNo FROM TLP " +
                    "WHERE EvrakNoTarih+EvrakNo='" + EvrakNumarasi + "'";

        cmd = new SqlCommand(sorgu, DbConnUser);
        return (string)cmd.ExecuteScalar();
    }

    private string CariHesapAdiBul(string EvrakNumarasi)
    {
        if (DbConnUser.State == ConnectionState.Closed)
            DbConnUser.Open();

        string Sorgu="SELECT ISNULL(CariUnvan,'C') AS CariUnvan FROM TLP " +
                    "WHERE EvrakNoTarih+EvrakNo='" + EvrakNumarasi + "'";
