...
public partial class Formlar_idariRaporlar : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            string TalepIDKontrol = Session["idariTalepID"] as string;

            if (!string.IsNullOrEmpty(TalepIDKontrol))
            {
                imgYaziciyaGonder.OnClientClick = "MM_openBrWindow('idariKisimihtiyacListesi.Aspx','','toolbar=yes,location=yes,status=yes,resizable=yes,scrollbars=yes,width=900,height=550')";
            }
            else
            {
                Session["idariTalepID"] = "1";
            }
            idariKisimTablomuz();
        }
    }

    private void idariKisimTablomuz()
    {
        ...
        string sorgu = "CREATE TABLE #idariKisimTablo( " +
                        "TalepID INT, " +
                        "EvrakNo NVARCHAR(50), " +
                        ...
                        "WHERE (T.idariKisim = 0) AND (CASE WHEN T.TalepBirim = '' THEN K.Birim " +
                        "ELSE T.TalepBirim END) = 'idari Birim' AND (T.GidecekDepartman='" + DepartmanKodu + "') AND (T.OnayDurumu NOT IN(9,10,11,12)) AND " +
                        ...
                        "S2.STK002_Miktari,T.DepartmanAdi,T.MasrafMerkeziAdi,T.Birim,S2.STK002_Tutari,CR.CAR002_Unvan1+CAR002_Unvan2,T.Aciklama+T.Aciklama2+T.Aciklama3 " +
                        "ORDER BY T.EvrakNoTarih+T.EvrakNo DESC " +
                        "SELECT * FROM #idariKisimTablo " +
                        "DROP TABLE #idariKisimTablo";

        da = new SqlDataAdapter(sorgu, DbConnLinkSorgu);
        da.Fill(ds, "idariKisimTablo");

        TabloSayisi = ds.Tables["idariKisimTablo"].Rows.Count;
        ...
        for (int i = 0; i < ds.Tables["idariKisimTablo"].Rows.Count; i++)
        {
            MalKodumuzNedir = MalKodumuz(ds.Tables["idariKisimTablo"].Rows[i]["EvrakNo"].ToString());

            if (MalKodumuzNedir.ToString().Substring(0, 1) == "M")
            {
                AciklamaSatirimiz = ds.Tables["idariKisimTablo"].Rows[i]["Aciklama"].ToString();
            }
            else
            {
                AciklamaSatirimiz = ds.Tables["idariKisimTablo"].Rows[i]["StokAdi"].ToString();
            }
            ...
            printer[i].Value = ds.Tables["idariKisimTablo"].Rows[i]["TalepID"].ToString() + "/" + ds.Tables["idariKisimTablo"].Rows[i]["BirimFiyat"].ToString();
            ...
            Lt1.Text = "<td align=\"center\" style=\"border:1px solid Black;\">" + ds.Tables["idariKisimTablo"].Rows[i]["EvrakNo"].ToString() + "</td> " +
                     "<td align=\"center\" style=\"border:1px solid Black;\">" + AciklamaSatirimiz + "</td> " +
                     "<td align=\"center\" style=\"border:1px solid Black;\">" + ds.Tables["idariKisimTablo"].Rows[i]["Miktar"].ToString().Replace('.', ',') + " " + ds.Tables["idariKisimTablo"].Rows[i]["Birim"].ToString() + "</td> " +
                     "<td align=\"center\" style=\"border:1px solid Black;\">" + ds.Tables["idariKisimTablo"].Rows[i]["BirimFiyat"].ToString().Replace('.', ',') + "</td> " +
                     "<td align=\"center\" style=\"border:1px solid Black;\">" + ds.Tables["idariKisimTablo"].Rows[i]["Tutar"].ToString().Replace('.', ',') + "</td> " +
                     "<td align=\"center\" style=\"border:1px solid Black;\">" + ds.Tables["idariKisimTablo"].Rows[i]["TalepEden"].ToString() + "</td> " +
                     "<td align=\"center\" style=\"border:1px solid Black;\">" + ds.Tables["idariKisimTablo"].Rows[i]["KullanilacakDepartman"].ToString() + "</td> " +
                     "<td align=\"center\" style=\"border:1px solid Black;\">" + ds.Tables["idariKisimTablo"].Rows[i]["Firma"].ToString() + "</td>";
    ...
    {
        Session["idariTalepID"] = null;
        Session["idariMalAciklama"] = null;
        Session["idariEvrakNo"] = null;
        Session["idariMalAdi"] = null;
        Session["idariTarih"] = null;
        Session["idariMiktar"] = null;
        Session["idariBirimFiyat"] = null;
        Session["idariTutar"] = null;
        Session["idariTalepEden"] = null;
        Session["idariKullanilacakYer"] = null;
        Session["idariFirma"] = null;
    ...
    private void TalepBul2(int TalepID)
    {
        ...
        if (dr.Read())
        {
            Session["idariTalepID"] += dr["TalepID"].ToString() + ",";
            Session["idariEvrakNo"] += dr["EvrakNo"].ToString() + ",";
            Session["idariMalAdi"] += dr["MalAdi"].ToString() + "}";
            Session["idariMalAciklama"] += dr["Aciklama"].ToString() + "}";
            Session["idariTarih"] += dr["Tarih"].ToString() + ",";
            Session["idariMiktar"] += dr["Miktar"].ToString() + "/";
            Session["idariBirim"] += dr["Birim"].ToString() + "/";
            Session["idariBirimFiyat"] += dr["BirimFiyat"].ToString() + "/";
            Session["idariTutar"] += dr["Tutar"].ToString() + "/";
            Session["idariTalepEden"] += dr["TalepEden"].ToString() + ",";
            Session["idariKullanilacakYer"] += dr["KullanilacakDepartman"].ToString() + ",";
            Session["idariFirma"] += dr["Firma"].ToString() + ",";
        }
